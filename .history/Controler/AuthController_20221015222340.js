...
exports.deleteUser = (async (req, res, next) => {
  try {
    // let if_containe_note=  await Product.findById({user: req.params.id});
    // let if_containe_work_time= await Excercise.findById({user:req.params.id});

   let if_containe_product=  await Product.findById({user: req.params.id.valueOf()});
  let if_containe_excercise= await Excercise.findById({user:req.params.id.valueOf()});
  if (if_containe_excercise && if_containe_product) {
    console.log(if_containe_excercise),
    console.log(if_containe_product),
    res.status(400).json({
      success: false,
